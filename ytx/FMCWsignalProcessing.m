...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%超分辨测角%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %% DML测角（理论上最佳？感觉看到的代码里面思路不对）
        %这里还是需要知道信源数量的

        %% OMP
        OMPresult=zeros(2,FFTnum(1));
        a_theta_OMP=exp((-1i*2*pi*radar.RxDis*sind(theta_scan)/radar.lambda).'*array);%角度格数*天线数
        a_theta_OMP=a_theta_OMP';
        K  = length(theta_scan);
        L  = 2;    %需要知道信源的个数(稀疏度)
        for frame=1:FFTnum(1)
            signal=squeeze(data(frame,:,:));
            for i=1:radar.sample_num
                lp=[];%这个应该叫什么？重建系数？
                signal_snap=signal(:,i);
                residual = signal_snap;      %初始化残差为信号
                index = zeros(L,1); %用来记录目标索引
                buffer=a_theta_OMP;%用来存储对应的备选的原子基向量 
                buffer_index=(1:K);%对应角度的索引后面用来查找角度
                for j=1:L
                    buffer=a_theta_OMP(:,setdiff(buffer_index,index));
                    proj = buffer'*residual;      %这里计算的是内积
                    pos  = find(abs(proj) == max(abs(proj)));%找到对应的内积最大的索引返回
                    index(j)=pos;
                    lp=pinv(buffer(:,index(1:j)))*signal_snap;
                    residual=signal_snap-buffer(:,index(1:j))*lp;%计算得到的残差
                end
                temp=zeros(K,1);
                temp(index)=lp;
                A(:,i)=sparse(temp);%得到对应的重建系数在对应的角度
            end
            A = full(A);
            for i= 1:size(signal,2)
                OMPresult(:,frame) = OMPresult(:,frame)+theta_scan(find(A(:,i)~=0))'/size(signal,2);
            end
        end
...
%         end
    %% 绘制各种测角算法的对比
        ...
        for i=1:FFTnum(1)
            ...
            scatter(OMPresult(:,i),[0,0],'filled');
            hold on;
